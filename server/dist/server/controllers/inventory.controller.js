...
exports.getMovementHistory = exports.recordMovement = exports.getStock = void 0;
...
const getStock = async (req, res) => {
    try {
        ...
        if (warehouseId)
            where.gudangId = String(warehouseId);
        if (productId) {
            const product = await prisma_1.default.produk.findUnique({
                where: { id: String(productId) },
                select: { persediaanId: true }
            });
            if (product === null || product === void 0 ? void 0 : product.persediaanId)
                where.persediaanId = product.persediaanId;
        }
        const stocks = await prisma_1.default.stokPersediaan.findMany({
            where,
            include: {
        ...
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            for (const item of validatedData.items) {
                ...
                if (['MASUK', 'ADJUSTMENT', 'TRANSFER'].includes(validatedData.tipe)) {
                    ...
                    if (!targetGudangId)
                        throw new Error('Gudang tujuan harus dipilih untuk Transfer');
...
const getMovementHistory = async (req, res) => {
    try {
        ...
        const [movements, total] = await Promise.all([
            prisma_1.default.mutasiPersediaan.findMany({
                where,
                include: {
            ...
            prisma_1.default.mutasiPersediaan.count({ where })
        ]);
        res.json({
            data: movements,
            pagination: {
