...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const login = async (data: Record<string, unknown>) => {
        try {
            ...
            const msg = axiosError.response?.data?.message || 'Login gagal. Cek email dan password Anda.';
            toast.error(msg);
            throw err;
        }
    };
    ...
    const registerUser = async (data: Record<string, unknown>) => {
        try {
            ...
            const axiosError = err as { response?: { data?: { message?: string } } };
            const msg = axiosError.response?.data?.message || 'Pendaftaran gagal. Silakan coba lagi.';
            toast.error(msg);
            throw err;
        }
    };
