...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside
        className={cn(
          "bg-white transition-all duration-300 flex flex-col z-50",
          isSidebarOpen ? "w-64" : "w-20"
        )}
      >
        <div className="h-16 flex items-center px-6 border-b">
          <div className="bg-primary h-8 w-8 rounded-lg flex items-center justify-center font-bold shrink-0">
            M
          </div>
          {isSidebarOpen && (
            <span className="ml-3 font-bold text-slate-900 tracking-tight transition-opacity duration-300">
              MGYT Accounting
            </span>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 sticky top-0 z-40 backdrop-blur-md">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="relative h-14 w-auto px-2 flex items-center gap-4 hover:bg-slate-50 rounded-full"
              >
